...
function textToCamelCase() {
  ...
  console.log(text);
  //split by enter and trim
  const textElements = text.split('\n');
  for (let i = 0; i < textElements.length; i++) {
    const subElements = textElements[i].trim().toLowerCase().split('_');
    let camelCaseStr = subElements[0];

    for (let j = 1; j < subElements.length; j++) {
      camelCaseStr += subElements[j][0].toUpperCase() + subElements[j].slice(1);
    }

    console.log(camelCaseStr.padEnd(20, ' '), sign.repeat(i + 1));
  }
}
