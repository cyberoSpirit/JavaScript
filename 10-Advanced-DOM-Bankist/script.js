...
const obsOptions = {
  root: null,
  threshold: 0, //how much of other staff can be visible
...
const secObsOptions = {
  root: null,
  threshold: 0.15, //how much of other staff can be visible
};
...
function goToSlide(currentSlide) {
  ...
  slides.forEach((s, i) => {
    s.style.transform = `translateX(${
      100 * Math.abs((i - currentSlide) % slides.length)
    }%)`;
  });
  activateDot(Math.abs(currentSlide % slides.length));
}
